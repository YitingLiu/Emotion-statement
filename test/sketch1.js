// var emotionTypes=["Joy","Sadness","Trust","Fear","Disgusting","Satisfied","Anger","Surprise","Anitcipatoin"];
var emotionTypes=[];
var emotionsFromDB;
...
function setup() {
	emotionsFromDB = emotionsFromDB.record;
	...
	emotionsFromDB.forEach(function(emotionRecord){
	  var currentEmotionsArrayFromDB=emotionRecord.emotions;
	  currentEmotionsArrayFromDB.forEach(function(currentEmotionFromDB){
	    if(emotionTypes.indexOf(currentEmotionFromDB)==-1){
	      //not in the array, add a new emotionTypes
	      emotionTypes.push(currentEmotionFromDB);
	    }
	  });
	});
	...
	var emotions=[];
	emotionTypes.forEach(function(e){
